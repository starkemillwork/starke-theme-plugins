<?php
	...
	if(
		...
		!isset($_POST) ||	// phpcs:ignore WordPress.Security.NonceVerification
		!isset($_POST['wsffid'])	// phpcs:ignore WordPress.Security.NonceVerification
	) {
		exit;
	}
	...
	// Read form ID
	$form_id =  absint($_POST['wsffid']);	// phpcs:ignore WordPress.Security.NonceVerification
	if($form_id === 0) { exit; }
	...
	WS_Form_Common::echo_json_encode(array('error' => false));
	exit;
	...
	function error($error_message) {
		...
		// JSON response
		header('Content-Type: application/json');
		WS_Form_Common::echo_json_encode(array('error' => true, 'error_message' => $error_message));
		exit;
	}
