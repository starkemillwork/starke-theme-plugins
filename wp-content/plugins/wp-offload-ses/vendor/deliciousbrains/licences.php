...
use DeliciousBrains\WP_Offload_SES\Licences\Delicious_Brains_API_Base;
use DeliciousBrains\WP_Offload_SES\Licences\Delicious_Brains_API_Updates;
...
abstract class Delicious_Brains_API_Licences extends Delicious_Brains_API_Base {
		...
		if ( is_null( $this->addons ) ) {
			...
			if ( isset ( $GLOBALS[ $meta_key ][ $this->plugin->slug ]['supported_addon_versions'] ) ) {
				...
				foreach ( $GLOBALS[ $meta_key ][ $this->plugin->slug ]['supported_addon_versions'] as $addon => $version ) {
					...
					if ( ! $installed ) {
						$basename = explode( '/', $this->get_plugin_basename( $slug ) );
						$folder   = $basename[0];
						$file     = $basename[1];
	...
	public function is_licence_expired( $skip_transient_check = false ) {
		...
		if ( empty( $licence ) ) {
			$settings_link = sprintf( '<a href="%s" class="js-action-link wposes-enter-licence">%s</a>', $this->admin_url( $this->plugin->settings_url_path ) . $this->plugin->settings_url_hash, __( 'enter your license key' ) );
	...
	function get_licence_status_message( $trans = false ) {
		...
		if ( isset( $errors['connection_failed'] ) ) {
			...
			$message .= sprintf( '%s <a href="#" class="js-action-link reactivate-licence">%s</a>', __( 'Your license has been deactivated for this install.', 'wp-migrate-db' ), __( 'Reactivate License' ) );
		} else {
			$error   = reset( $errors );
	...
	public function ajax_remove_licence() {
		...
		if ( $this->is_licence_constant() ) {
			wp_send_json_error( array(
				'message' => __( 'Your licence key is currently defined via a constant and must be removed manually.', 'wp-offload-ses' ),
			) );
		}
		...
		wp_send_json_success( array(
			'message' => __( 'Licence key removed successfully.', 'wp-offload-ses' ),
		) );
	}
	...
	public function ajax_check_licence() {
		...
		if ( ! empty( $decoded_response['dbrains_api_down'] ) ) {
			...
			if ( ! $help_message ) {
				...
				<p>
					<strong><?php _e( 'Please copy the Diagnostic Info &amp; Error Log info below into a text file and attach it to your email.' ); ?></strong>
				</p>
				<p class="email">
